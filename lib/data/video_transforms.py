...
class GroupRandomCrop(object):
    def __init__(self, size):
        short_edge = int(round(np.random.uniform(256, 320)))
        self.worker = GroupScale(short_edge)
    ...
    def __call__(self, Te):
        img_group, label = Te
        # tic = time.time()
        # img_group = self.worker(img_group)
        ...
        # print('crop time:%.4f'%(time.time()-tic))
...
class GroupNormalize(object):
    ...
    def __call__(self, Te):
        ...
        # TODO: improve efficiency in following block
        for t, m, s in zip(tensor, rep_mean, rep_std):
            t.sub_(m).div_(s)

        return tensor.view(org_shape), label
...
class GroupScale(object):
    ...
    def __init__(self, size, interpolation=Image.BILINEAR, keep_ratio = 0):
    ...
    def __call__(self, img):
        ...
        if self.keep_ratio:
            ...
            return [img.resize((320, 256), self.interpolation) for img in img_group], label
...
class GroupFCNSample(object):
    def __init__(self, crop_size):
        self.scale_worker = GroupScale(256,keep_ratio=1)
    ...
    def __call__(self, img, is_flow=False):
        ...
        for o_w, o_h in offsets:
            ...
            for i, img in enumerate(img_group):
                ...
                normal_group.append(crop)
            oversample_group.extend(normal_group)
        return oversample_group, label  # , None


class GroupFCNSample_0(object):
    def __init__(self, crop_size, scale_size=256):
        self.crop_size = crop_size if not isinstance(crop_size, int) else (crop_size, crop_size)
        self.scale_worker = GroupScale(scale_size)
    ...
    def __call__(self, img):
        ...
        for o_w in offsets:
            ...
            flip_group = list()
            for i, img in enumerate(img_group):
                crop = img.crop((o_w, 0, o_w + 256, 256))
                normal_group.append(crop)
...
class GroupMultiScaleCrop(object):
    ...
    def __init__(self, input_size, scales=None, max_distort=1, fix_crop=True, more_fix_crop=True):
        ...
        self.interpolation = Image.BILINEAR
        self.scale_worker = GroupScale(256)
...
class GroupFixedRandomCrop(object):
    ...
    def __call__(self, img):
        img_group, label = img
        img_group = [img.resize((320, 256), self.interpolation)
                     for img in img_group]

        im_size = img_group[0].size

        crop_w, crop_h, offset_w, offset_h = self._sample_crop_size(im_size)
        ret_img_group = [img.crop((offset_w, offset_h, offset_w + crop_w, offset_h + crop_h)) for img in img_group]
        return ret_img_group, label
...
class Stack(object):
    ...
    def __call__(self, img):
        ...
        if img_group[0].mode == 'L':
            ...
            if self.roll:
                return np.array([np.array(x)[:, :, ::-1] for x in img_group]), label
            else:
                return np.array([np.array(x) for x in img_group]), label
...
class ToTorchFormatTensor(object):
    ...
    def __call__(self, pi):
        ...
        if isinstance(pic, np.ndarray):
            ...
            # TODO: improve this transpose; currently takes 80% of the loading time/CPU
