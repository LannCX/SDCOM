...
class BaseConfig(object):
    def __init__(self, args=None):
        ...
        _C.MODEL = CN()
        _C.MODEL.NAME = ''
        _C.MODEL.DROPOUT = 0.5  # dropout probability for linear layer
        ...
        _C.TRAIN.DROP_LAST = True  # drop the last mini-batch
        _C.TRAIN.ACCUM_N_BS = 1  # accumulate losses for N steps (mini-batch size)
        _C.TRAIN.SMOOTHING = 0.1  # label smoothing

        # testing or validation
        _C.TEST = CN()
        _C.TEST.VAL_FREQ = 1  # test frequency
        _C.TEST.START_VAL_EPOCH = 1  # validate start from this epoch
