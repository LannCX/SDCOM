...
from arch.ops import DyChConv2d, DyChBatchNorm2d, DyChLinear, DyShiftModule
from utils.helpers import efficientnet_init_weights
...
class DyChBottleneck(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None, norm_layer=None, n_segment=8):
        super(DyChBottleneck, self).__init__()
        self.shift = DyShiftModule(inplanes, n_segment=n_segment)
...
class DyChResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=True, n_segment=8):
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, DyChBottleneck) :
                    m.init_residual_norm()
...
def dytsmresnet50(pretrained=False, **kwargs):
